...
Стандарт также определяет `<<` для [[complex|complex]], [[bitset|bitset]], кодов ошибок и указателей. Потоковый ввод-вывод является расширяемым, поэтому мы можем  [[iostream#I/O пользовательских типов|определить `<<` для наших собственных (определяемых пользователем) типов]].
...
Подобно форматировщикам потоков, `format()` также может обрабатывать время и даты ([[Утилиты-STL#Календари|Календари]]). Например:
```c++
