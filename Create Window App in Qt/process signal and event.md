...
С помощью метода `setFocusProxy()` из класса [[QGraphicsItem|QGraphicsItem]] можно указать объект, который будет обрабатывать события клавиатуры вместо текущего объекта. Получить указатель на такой объект позволяет метод `focusProxy()`. Прототипы методов:
```c++
...
С помощью метода `setAcceptedMouseButtons()` из класса [[QGraphicsItem|QGraphicsItem]] можно указать кнопки, события от которых объект принимает. По умолчанию объект принимает события от всех кнопок мыши. Если в параметре `buttons` указать константу `NoButton`, то обработка событий мыши будет отключена. Прототип метода:
```c++
...
Класс `QGraphicsSceneMouseEvent` наследует все методы из классов [[GraphicsSceneEvent|GraphicsSceneEvent]] и [[QEvent|QEvent]] и добавляет следующие методы:

> **pos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами в пределах области объекта. Прототип метода:
```c++
QPointF pos() const
```

> **scenePos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами в пределах сцены. Прототип метода:
```c++
QPointF scenePos() const
```

> **lastPos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами последней запомненной представлением позиции в пределах области объекта. Прототип метода:
```c++
QPointF lastPos() const
```

> **lastScenePos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами последней запомненной представлением позиции в пределах сцены. Прототип метода:
```c++
QPointF lastScenePos() const
```

> **lastScreenPos()** — возвращает экземпляр класса [[QPoint#QPoint|QPoint]] с координатами последней запомненной представлением позиции в пределах экрана. Прототип метода:
```c++
QPoint lastScreenPos() const
```

> **buttonDownPos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами щелчка указанной кнопки мыши в пределах области объекта. Прототип метода:
```c++
QPointF buttonDownPos(Qt::MouseButton button) const
```

> **buttonDownScenePos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами щелчка указанной кнопки мыши в пределах сцены. Прототип метода:
```c++
QPointF buttonDownScenePos(Qt::MouseButton button) const
```

> **buttonDownScreenPos()** — возвращает экземпляр класса [[QPoint#QPoint|QPoint]] с координатами щелчка указанной кнопки мыши в пределах экрана. Прототип метода:
```c++
...
По умолчанию событие мыши перехватывает объект, над которым произведен щелчок мышью. Чтобы перехватывать нажатие и отпускание мыши вне объекта, следует захватить мышь с помощью метода `grabMouse()` из класса [[QGraphicsItem|QGraphicsItem]]. Освободить захваченную ранее мышь позволяет метод `ungrabMouse()`. Получить указатель на объект, захвативший мышь, можно с помощью метода `mouseGrabberItem()` из класса [[QGraphicsScene|QGraphicsScene]]. Прототип метода:
```c++
...
> **screenPos()** — возвращает экземпляр класса [[QPoint#QPoint|QPoint]] с координатами в пределах экрана. Прототип метода:
```c++
...
Класс `QGraphicsSceneWheelEvent` наследует все методы из классов  [[GraphicsSceneEvent|GraphicsSceneEvent]] и [[QEvent|QEvent]] и добавляет следующие методы:
...
> **pos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами указателя мыши в пределах области объекта. Прототип метода:
```c++
QPointF pos() const
```

> **scenePos()** — возвращает экземпляр класса [[QPoint#QPointF|QPointF]] с координатами указателя мыши в пределах сцены. Прототип метода:
```c++
QPointF scenePos() const
```

> **screenPos()** — возвращает экземпляр класса [[QPoint#QPointF|QPoint]] с координатами указателя мыши в пределах экрана. Прототип метода:
```c++
QPoint screenPos() const
```
...
Прежде чем обрабатывать перетаскивание и сброс, необходимо сообщить системе, что графический объект может обрабатывать эти события. Для этого следует вызвать метод `setAcceptDrops()` из класса [[QGraphicsItem|QGraphicsItem]] и передать ему значение `true`. Прототип метода:
```c++
void setAcceptDrops(bool on)
```
...
Класс `QGraphicsSceneDragDropEvent` наследует все методы из классов [[GraphicsSceneEvent|QGraphicsSceneEvent]] и [[QEvent|QEvent]] и добавляет следующие методы:
...
Указать, события какого объекта фильтруются, позволяют следующие методы из
класса [[QGraphicsItem|QGraphicsItem]]:
