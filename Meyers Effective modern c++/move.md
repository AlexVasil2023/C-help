...
В качестве небольшого отступления скажем, что `std::move` можно реализовать в С++14 меньшими усилиями. Благодаря [[decltуре|выводу возвращаемого типа функции]] и шаблону псевдонима [[remove_reference|std::remove_reference_t]]  `std::move` можно записать следующим образом:
```c++
