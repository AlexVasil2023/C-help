...
>	> Если `е` представляет собой [[template значение#l-значение|l-значение]] типа `Т`, `decltype (е)` дает тип `Т&`.
>	> Если `е` представляет собой [[template значение#x-значения|х-значение]] типа `Т`, `decltype (е)` дает тип `Т&&`.
>	> Если `е` представляет собой [[template значение#pr-значения|рr-значение]] типа `Т`, `decltype (е)` дает тип `Т`.
...
Наше выражение ссылается на переменную по имени и, таким образом, является [[template значение#l-значение|l-значением]]: согласно приведенным выше правилам, это означает, что `decltype(s)` является обычной ссылкой (т.е. ссылкой на [[template значение#l-значение|l-значение]]) на [[string|std::string]] (поскольку тип `(s)` — [[string|std::string]]). Это одно из немногих мест в C++, где скобки меняют смысл программы (помимо воздействия на порядок вычислений операторов).
...
Еще один пример демонстрирует разницу при индексации [[vector|std::vector]] (дающем [[template значение#l-значение|l-значение]]):
```c++
std::vector<int> v = { 42 };
...
Если `container[idx]` дает [[template значение#l-значение|l-значение]], мы хотим передать его вызывающему коду как [[template значение#l-значение|l-значение]] (кто-то может захотеть получить его адрес или изменить его). Это требует тип ссылки на [[template значение#l-значение|l-значение]], в который разрешается конструкция `decltype (auto)`. Если же вместо этого получается [[template значение|pr-значение]], ссылочный тип приведет к висячим ссылкам, — но, к счастью, для этого случая `decltype (auto)` будет давать тип объекта (а не ссылочный тип).
...
Обратите также внимание на то, что скобки в инициализаторе могут быть значимыми ([[template Семантика перемещения - Прямая передача|поскольку они являются существенными]] для конструкции [[decltуре#decltype C++11|decltype]]):
```c++
int х;
...
Начиная с C++17, `decltype (auto)` может также использоваться для [[template Вывод аргументов шаблона#Спецификатор типа auto|выводимых параметров, не являющихся типами]]. Это проиллюстрировано в приведенном ниже примере:
```c++
