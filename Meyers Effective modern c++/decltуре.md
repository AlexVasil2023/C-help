...
>	> Если `е` представляет собой [[r-значение#l-значение|l-значение]] типа `Т`, `decltype (е)` дает тип `Т&`.
>	> Если `е` представляет собой [[r-значение#x-значения|х-значение]] типа `Т`, `decltype (е)` дает тип `Т&&`.
>	> Если `е` представляет собой [[r-значение#pr-значения|рr-значение]] типа `Т`, `decltype (е)` дает тип `Т`.

О категориях значений рассказывается в [[r-значение|“Категории значений”]].
...
Наше выражение ссылается на переменную по имени и, таким образом, является [[r-значение#l-значение|l-значением]]: согласно приведенным выше правилам, это означает, что `decltype(s)` является обычной ссылкой (т.е. ссылкой на [[r-значение#l-значение|l-значение]]) на [[string|std::string]] (поскольку тип `(s)` — [[string|std::string]]). Это одно из немногих мест в C++, где скобки меняют смысл программы (помимо воздействия на порядок вычислений операторов).
...
Еще один пример демонстрирует разницу при индексации [[vector|std::vector]] (дающем [[r-значение#l-значение|l-значение]]):
```c++
std::vector<int> v = { 42 };
...
Если `container[idx]` дает [[r-значение#l-значение|l-значение]], мы хотим передать его вызывающему коду как [[r-значение#l-значение|l-значение]] (кто-то может захотеть получить его адрес или изменить его). Это требует тип ссылки на [[r-значение#l-значение|l-значение]], в который разрешается конструкция `decltype (auto)`. Если же вместо этого получается [[r-значение|pr-значение]], ссылочный тип приведет к висячим ссылкам, — но, к счастью, для этого случая `decltype (auto)` будет давать тип объекта (а не ссылочный тип).
