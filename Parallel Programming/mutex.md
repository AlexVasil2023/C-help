...
4. [[#std recursive_mutex|std::recursive_mutex]]
...
5. Если функция `getVar` выбросит исключение, мьютекс `m` никогда не будет освобождён.
6. Никогда, ни в коем случае нельзя вызывать из-под мьютекса функцию, внутреннее устройство которой неизвестно. Если функция `getVar` пытается захватить мьютекс `m`, поведение программы не определено, так как этот мьютекс – не рекурсивный. В большинстве подобных случаев неопределённое поведение выражается в мёртвой блокировке.
7. Вызывать функции из-под блокировки опасно ещё по одной причине. Если функция определена в сторонней библиотеке, в новой её версии реализация и поведение функции могут измениться. Даже если первоначально опасности мёртвой блокировки не было, она может появиться в будущем.
