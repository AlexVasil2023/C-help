...
>
> Когда мы передаем имя функции в качестве аргумента функции, мы в действительности передаем не саму функцию, а указатель или ссылку на нее. [[template По значению или по ссылке#Работа со строковыми литералами и массивами|Как и в случае массивов]], аргумент-функция низводится до указателя при передаче по значению, а в случае параметра, тип которого является параметром шаблона, будет выведен тип указателя на функцию.
...
```
> Обратите внимание на то, что при определении `operator()` вы обычно должны определять как константную функцию-член. В противном случае [[template Применение шаблонов на практике#Расшифровка романов об ошибках|может произойти тонкая ошибка, когда каркасы или библиотеки ожидают, что такой вызов не изменяет состояние переданного объекта]].
...
Еще одним интересным аспектом является работа с возвращаемым значением вызываемой функции для его “прямой передачи” назад вызывающему коду. Для поддержки возвращения ссылок (как, например, [[stream|std::ostream&]]) следует использовать [[decltуре#decltype(auto) C++14|decltype(auto)]] вместо просто [[auto|auto]]:
```c++
template<typename Callable, typename... Args>
decltype(auto) call(Callable&& op, Args&& ... args)
```
...
Как показывает этот пример, путем проверки некоторых условий мы можем выбирать между различными реализациями шаблона. Здесь мы используем [[template Программирование времени компиляции#Инструкция if времени компиляции|функциональную возможность if времени компиляции, которая появилась в C++17]], но вместо нее могли бы [[template Программирование времени компиляции#Программирование времени компиляции|использовать std::enable_if, частичную специализацию или SFINAE для включения или отключения вспомогательных шаблонов]].
...
Здесь мы могли бы ожидать, что [[add_rvalue_reference|add_rvalue_reference]] всегда дает в результате [[template значение#r-значения|r-ссылки]], но правила свертывания ссылок C++ (см. #раздел_15_6_1) приводят к тому, что сочетание [[template значение|l-ссылки и r-ссылки дает l-ссылку]]. 
...
[[template Семантика перемещения - Прямая передача|Как было показано]], можно использовать передаваемые ссылки и [[forward|std::forward<>]] для “прямой передачи” обобщенных параметров:
```c++
template<typename Т>
...
Такая [[template Применение шаблонов на практике#Расшифровка романов об ошибках|ошибка часто приводит к “романам”]], в которых компилятор выводит всю историю инстанцирования шаблона от исходного шаблона до фактического шаблона, в котором была обнаружена ошибка.
...
Ссылочные типы в целом довольно сильно отличаются от других типов, и к ним применяются несколько уникальных языковых правил. Они воздействуют, например, на [[template По значению или по ссылке|объявления параметров вызова]], а также на способы определения свойств типов (см. #раздел_19_6_1).

# Откладывание вычислений

При реализации шаблонов иногда возникает вопрос, может ли код справиться с [[template Основные термины в области шаблонов#Полные и неполные типы|неполными типами]]. Рассмотрим следующий шаблон класса:
```c++
template<typename Т>
class Cont
{
...
Здесь мы используем свойство [[conditional|std::conditional]] (см. #раздел_Г_5) для выяснения, является ли возвращаемым типом функции-члена `foo()` тип `Т&&` или `Т&`. Решение зависит от того, поддерживает ли тип параметра шаблона `Т` семантику перемещения.

Проблема заключается в том, что свойство [[is_move_constructible|std::is_move_constructible]] требует, чтобы ее аргумент был полным типом (а также не типом [[void|void]] и не массивом с неизвестными границами; см. #раздел_Г_3_2). Поэтому при таком объявлении `foo()` объявление `struct node` является некорректным6.
...
>
> Используйте передаваемые [[template Семантика перемещения - Прямая передача|ссылки для передачи значений в шаблоны]]. Если значения не зависят от параметров шаблона, [[template Обобщенные библиотеки#Оборачивание вызовов функций|используйте auto&&]].
> 
> Когда параметры объявлены как передаваемые ссылки, будьте готовы к тому, что параметр шаблона будет иметь ссылочный тип при передаче l-значений (см. #раздел_15_6_2).
> 
...
> 
> Используя шаблоны функций-членов, убедитесь, что их соответствие не лучше, чем [[template Семантика перемещения - Использование enable_if|предопределенные копирующий/перемещающий конструктор или оператор присваивания]].
> 
> Подумайте об использовании [[decay|std::decay]], когда параметры шаблона могут быть [[template По значению или по ссылке#Работа со строковыми литералами и массивами|строковыми литералами, не передающимися по значению]] ( #раздел_Г_4).
> 
> При наличии [[template По значению или по ссылке#Передача с помощью неконстантной ссылки|выходных параметров, зависящих от параметров шаблона, будьте готовы справиться с ситуацией, когда могут быть указаны константные аргументы шаблона]].
> 
> Будьте готовы справиться с побочными эффектами [[template Обобщенные библиотеки#Ссылки в качестве параметров шаблонов|параметров шаблона, которые представляют собой ссылки]] (пример в #разделе_19_6_1). В частности, может потребоваться гарантировать, что [[template По значению или по ссылке#Работа с возвращаемыми значениями|возвращаемый тип не может стать ссылкой]].
> 
> Будьте готовы к работе с неполными типами для поддержки, например, [[template Обобщенные библиотеки#Откладывание вычислений|рекурсивных структур данных]].
> 
> Выполняйте [[template Шаблоны для массивов и строковых литералов|перегрузки для всех типов массивов, а не только Т[SZ] ]].
