...
Обратите внимание на то, что другие свойства типов иногда также неявно выполняют низведение, как, например, [[common_type|std: :common_type<>]], который [[Шаблоны функций#Возвращаемый тип как общий тип|дает общий тип для двух переданных в качестве аргументов типов]] (см. #раздел_Г_5 (раздел 1.3.3)).
...
Здесь `arg1` и `arg2` должны быть обычными массивами элементов одного и того же типа `Т`, но с разными размерами `L1` и `L2`. Заметим, однако, что может потребоваться [[Шаблоны для массивов и строковых литералов|несколько реализаций для поддержки массивов различных видов]].
...
> Позволить компилятору [[Шаблоны функций#Вывод возвращаемого типа|выводить тип возвращаемого значения]], просто объявляя возвращаемый тип как [[auto|auto]] (начиная с C++14), поскольку при использовании [[auto|auto]] всегда выполняется низведение типа:
```c++
template<typename Т>
