...
Введенные [[Реализация свойств типов - template#Свойства-предикаты|свойства-предикаты]], возвращающие логическое значение, должны возвращать значение, производное от `std::true_type` или `std::false_type`. Таким способом можно также решить проблему, связанную с тем, что на некоторых платформах `sizeof (char) == sizeof (long)`.
...
Однако некоторые свойства, представленные в книге к этому моменту (например, [[Реализация свойств типов - template#Свойства типов результатов|свойство PlusResultT]]), ведут себя при наличии ошибок не так хорошо. Вспомним определение `PlusResultT` из указанного раздела:
```c++
#include <utility>
