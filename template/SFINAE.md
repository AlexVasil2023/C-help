...
Однако некоторые свойства, представленные в книге к этому моменту (например, [[template Реализация свойств типов#Свойства типов результатов|свойство PlusResultT]]), ведут себя при наличии ошибок не так хорошо. Вспомним определение `PlusResultT` из указанного раздела:
```c++
#include <utility>
